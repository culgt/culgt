...
/**
 * HV: added for dot product reduction
 * @param g_idata
 * @param g_idata2
 * @param g_odata
 * @param n
...
template <class T, unsigned int blockSize, bool nIsPow2>
__global__ void
reduce6abs(T *g_idata, T *g_odata, unsigned int n)
{
    T *sdata = SharedMemory<T>();
    ...
    while (i < n)
    {
    	mySum += (g_idata[i] > (T)0.0)?(g_idata[i]):(-g_idata[i]);

        // ensure we don't read out of bounds -- this is optimized away for powerOf2 sized arrays
        if (nIsPow2 || i + blockSize < n)
        	mySum += (g_idata[i+blockSize] > (T)0.0)?(g_idata[i+blockSize]):(-g_idata[i+blockSize]);
...
template <class T, unsigned int blockSize, bool nIsPow2>
__global__ void
reduce6max(T *g_idata, T *g_odata, unsigned int n)
{
    T *sdata = SharedMemory<T>();
    ...
    while (i < n)
    {
    	if (g_idata[i] > mySum) mySum = g_idata[i];

        // ensure we don't read out of bounds -- this is optimized away for powerOf2 sized arrays
        if (nIsPow2 || i + blockSize < n)
        	if (g_idata[i+blockSize] > mySum) mySum = g_idata[i+blockSize];
    ...
    if (tid < 32)
    {
        ...
        if (blockSize >=  64)
        {
        	if( smem[tid + 32] > mySum )
        		smem[tid] = mySum = smem[tid + 32];
        }

        if (blockSize >=  32)
        {
        	if( smem[tid + 16] > mySum )
...
template <class T>
void
reduceabs(int size, int threads, int blocks,
       int whichKernel, T *d_idata, T *d_odata)
{
    dim3 dimBlock(threads, 1, 1);
    dim3 dimGrid(blocks, 1, 1);
...
//            break;
        ...
        default:
            if (isPow2(size))
            {
                ...
                {
                    case 512:
                        reduce6abs<T, 512, true><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                    case 256:
                    	reduce6abs<T, 256, true><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                    case 128:
                    	reduce6abs<T, 128, true><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                    case 64:
                    	reduce6abs<T,  64, true><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                    case 32:
                    	reduce6abs<T,  32, true><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                    case 16:
                    	reduce6abs<T,  16, true><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                    case  8:
                    	reduce6abs<T,   8, true><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                    case  4:
                    	reduce6abs<T,   4, true><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                    case  2:
                    	reduce6abs<T,   2, true><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                    case  1:
                    	reduce6abs<T,   1, true><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                }
            }
            else
            {
                ...
                {
                    case 512:
                    	reduce6abs<T, 512, false><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                    case 256:
                    	reduce6abs<T, 256, false><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                    case 128:
                    	reduce6abs<T, 128, false><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                    case 64:
                    	reduce6abs<T,  64, false><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                    case 32:
                    	reduce6abs<T,  32, false><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                    case 16:
                    	reduce6abs<T,  16, false><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                    case  8:
                    	reduce6abs<T,   8, false><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                    case  4:
                    	reduce6abs<T,   4, false><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                    case  2:
                    	reduce6abs<T,   2, false><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                    case  1:
                    	reduce6abs<T,   1, false><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                }
            }
            break;
    }
...
template <class T>
void
reducemax(int size, int threads, int blocks,
       int whichKernel, T *d_idata, T *d_odata)
{
    dim3 dimBlock(threads, 1, 1);
    dim3 dimGrid(blocks, 1, 1);
...
//            break;
        ...
        default:
            if (isPow2(size))
            {
                ...
                {
                    case 512:
                        reduce6max<T, 512, true><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                    case 256:
                    	reduce6max<T, 256, true><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                    case 128:
                    	reduce6max<T, 128, true><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                    case 64:
                    	reduce6max<T,  64, true><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                    case 32:
                    	reduce6max<T,  32, true><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                    case 16:
                    	reduce6max<T,  16, true><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                    case  8:
                    	reduce6max<T,   8, true><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                    case  4:
                    	reduce6max<T,   4, true><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                    case  2:
                    	reduce6max<T,   2, true><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                    case  1:
                    	reduce6max<T,   1, true><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                }
            }
            else
            {
                ...
                {
                    case 512:
                    	reduce6max<T, 512, false><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                    case 256:
                    	reduce6max<T, 256, false><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                    case 128:
                    	reduce6max<T, 128, false><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                    case 64:
                    	reduce6max<T,  64, false><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                    case 32:
                    	reduce6max<T,  32, false><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                    case 16:
                    	reduce6max<T,  16, false><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                    case  8:
                    	reduce6max<T,   8, false><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                    case  4:
                    	reduce6max<T,   4, false><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                    case  2:
                    	reduce6max<T,   2, false><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                    case  1:
                    	reduce6max<T,   1, false><<< dimGrid, dimBlock, smemSize >>>(d_idata, d_odata, size); break;
                }
            }
            break;
    }
...
//
//template void
//reducedot<float>(int size, int threads, int blocks,
//              int whichKernel, float *d_idata, float *d_idata2, float *d_odata);
//
//template void
